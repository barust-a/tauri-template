...
import { SupportedFile, DropZoneState } from '../../../types/fileTypes';
  ...
  const handleDrop = useCallback(async (e: React.DragEvent<HTMLDivElement>) => {
    ...
    for (const file of files) {
      ...
      if (validatedFile.isValid && validatedFile.fileType) {
        supportedFiles.push(validatedFile.fileType);
      }
    }
    ...
    setState(prev => ({
      ...
      validationResults: files.map(file => {
        ...
        return {
          ...validation,
          fileType: validation.isValid ? supportedFiles.find(sf => sf.name === file.name) : undefined
        };
      })
    }));
    ...
    if (supportedFiles.length > 0) {
      // Emit Tauri event for file processing
      try {
        if (typeof window !== 'undefined' && (window as any).__TAURI__) {
          (window as any).__TAURI__.event.emit('file-dropped', {
            files: supportedFiles,
            timestamp: new Date().toISOString()
          });
        }
      } catch (error) {
        console.warn('Tauri event emission failed:', error);
      }
    ...
    <div className="w-full max-w-2xl mx-auto p-6">
      <div className="bg-white rounded-lg shadow-lg border-2 border-dashed border-gray-300 p-8 text-center">
        <div className="space-y-4">
          ...
          <div
            className={`relative border-2 border-dashed rounded-lg p-8 transition-all duration-200 ${state.isDragOver
          ...
          {state.validationResults.length > 0 && (
            <div className="mt-6 space-y-2">
              ...
              {state.validationResults.map((result, index) => (
                <div
                  key={index}
                  className={`flex items-center justify-between p-3 rounded-md ${result.isValid ? 'bg-green-50' : 'bg-red-50'
                    }`}
                >
